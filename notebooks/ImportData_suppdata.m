...
elseif desktop
    cd 'F:\Data\OceanProductivity'
    filebase = 'F:\Data\OceanProductivity';
    if algo_choice == 'chl'
        filebase = 'F:\Data\OceanProductivity\Chl';
        cd Chl\
    elseif algo_choice =='mld'
        filebase = 'F:\Data\OceanProductivity\MLD';
        cd MLD\
    end
end
...
% chl_all_8day=NaN*ones(1080,1380,length(D0)); %(1080,2160,length(D0)); 
mld_all_8day=NaN*ones(1080,1380,length(D0)); %(1080,2160,length(D0)); 
...
for iix=1:length(D0)
    ...
    B=hdfread(list,'mld');% import data ** if no scaling equation, and importing whole region, can technically add B straight to "algo"_npp_all
    ...
    mld_all_8day(:,:,iix)=C; %C or B for whole region
end
fill=cell2mat(hdfread(b{1,1},'Hole Value'))  
mld_all_8day(mld_all_8day==fill)=NaN;
figure; pcolor(lonC,latC,mld_all_8day(:,:,863)); shading flat

clearvars -except mld* chl* algo_choice D0 filebase b fill
...
for ii = 1:1080
    ...
    %% Calculate area of boxes
        ...
        disp(['...'])
        area_MODIS_km2=NaN*ones(1080,4320);
        ...
        for aix = 1:length(latpixelnorthedge)
            ...
            area_MODIS_km2(aix,1)=areaint([lat_s,lat_n,lat_n,lat_s,lat_s],[lon_w,lon_w,lon_e,lon_e,lon_w],earthgeoid);
        end
        
        for ii = 2:4320
            area_MODIS_km2(:,ii)=area_MODIS_km2(:,1);
        end
        % convert to m^2
        area_MODIS_m2 = area_MODIS_km2 * 1e6;

% reduce to just wg for computing/matlab size
lon_wg=lon_m(:,1261:2640);
lat_wg=lat_m(:,1261:2640);
area_MODIS_km2_wg=area_MODIS_km2(:,1261:2640);
area_MODIS_m2_wg=area_MODIS_m2(:,1261:2640);
...
clearvars area_MODIS_m2 area_MODIS_km2 lon_m lat_m ii

%% sanity check

figure; pcolor(lon_wg,lat_wg,mld_all_8day(:,:,843)); shading flat
...
save mld_8day_imported_May22 -v7.3
...
mld_all_8day=cat(3,mld_all_8day,cafe_new);
...
save('cafe_8day_imported_May22.mat','algo_choice','area_MODIScafe_km2_wg','area_MODIScafe_m2_wg',...
    'mld_all_8day','cafe_npp_tot_gC_all_8day','cafe_npp_tot_gC_nans_8day','filebase','fill',...
