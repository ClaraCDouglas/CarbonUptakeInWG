...
%             eval(['temp.area_MODIS=area_MODIS',algorithm{aix},'_m2;']);
%             OceanProd.(algorithm{aix}).(region_sublist{rix}).area_month_m2(tix,1)=nansum(nansum((temp.NPPmask).*(temp.area_MODIS).*(temp.(region_sublist{rix}).box)));
            OceanProd.(algorithm{aix}).(region_sublist{rix}).area_month_m2(tix,1)=nansum(nansum(NPPmask.*area_MODIScafe_m2.*(temp.(region_sublist{rix}).box)));
            OceanProd.(algorithm{aix}).(region_sublist{rix}).area_month_km2=OceanProd.(algorithm{aix}).(region_sublist{rix}).area_month_m2/1e6;
        end
    end
end
 ...
 for aix = 1:length(algorithm)
     for rix = 1:length(region_sublist)
         for yix = 2003:2020
             ...
             % Integrated NPP over austral year (June to June)
             % and total and mean annual open ice-free water area and max monthly
             OceanProd.(algorithm{aix}).(region_sublist{rix}).NPP_tot_TgC_annual(yix-2002,1)=yix;
             OceanProd.(algorithm{aix}).(region_sublist{rix}).IceFree_annualTOT(yix-2002,1)=yix;
             OceanProd.(algorithm{aix}).(region_sublist{rix}).IceFree_annualMEAN(yix-2002,1)=yix;
             OceanProd.(algorithm{aix}).(region_sublist{rix}).IceFree_annualMAX(yix-2002,1)=yix;
             
             % and annual rates of NPP 
             OceanProd.(algorithm{aix}).(region_sublist{rix}).NPP_rates_fromdaily(yix-2002,1)=yix;
             OceanProd.(algorithm{aix}).(region_sublist{rix}).NPP_rates_frommonth(yix-2002,1)=yix;
             OceanProd.(algorithm{aix}).(region_sublist{rix}).NPP_rates_fromannual(yix-2002,1)=yix;
             OceanProd.(algorithm{aix}).(region_sublist{rix}).NPP_rates_yearmaxOW(yix-2002,1)=yix;
             
             if size(findyear)<46
                 OceanProd.(algorithm{aix}).(region_sublist{rix}).NPP_tot_TgC_annual(yix-2002,2)=NaN;
                 OceanProd.(algorithm{aix}).(region_sublist{rix}).IceFree_annualTOT(yix-2002,2)=NaN;
                 OceanProd.(algorithm{aix}).(region_sublist{rix}).IceFree_annualMEAN(yix-2002,2)=NaN;
                 OceanProd.(algorithm{aix}).(region_sublist{rix}).IceFree_annualMAX(yix-2002,2)=NaN;
                 
                 OceanProd.(algorithm{aix}).(region_sublist{rix}).NPP_rates_fromdaily(yix-2002,2)=NaN;
                 OceanProd.(algorithm{aix}).(region_sublist{rix}).NPP_rates_frommonth(yix-2002,2)=NaN;
                 OceanProd.(algorithm{aix}).(region_sublist{rix}).NPP_rates_fromannual(yix-2002,2)=NaN;
                 OceanProd.(algorithm{aix}).(region_sublist{rix}).NPP_rates_yearmaxOW(yix-2002,2)=NaN;
             else
                 OceanProd.(algorithm{aix}).(region_sublist{rix}).NPP_tot_TgC_annual(yix-2002,2)=sum(OceanProd.(algorithm{aix}).(region_sublist{rix}).NPP_tot_TgC(findyear,1));
                 OceanProd.(algorithm{aix}).(region_sublist{rix}).IceFree_annualTOT(yix-2002,2)=sum(OceanProd.(algorithm{aix}).(region_sublist{rix}).area_month_km2(findyear));
                 OceanProd.(algorithm{aix}).(region_sublist{rix}).IceFree_annualMEAN(yix-2002,2)=OceanProd.(algorithm{aix}).(region_sublist{rix}).IceFree_annualTOT(yix-2002,2)/12;
                 OceanProd.(algorithm{aix}).(region_sublist{rix}).IceFree_annualMAX(yix-2002,2)=max(OceanProd.(algorithm{aix}).(region_sublist{rix}).area_month_m2(findyear,1),[],'omitnan');
